...
send:
    switch (state) {
        ...
        case SYN_ACKED: {
            ...
            //         it is necessary
            if (to_send_sz == 0 || _window_size == 0)
                return;
            ...
            // size = to_send_sz < _window_size ? (to_send_sz < TCPConfig::MAX_PAYLOAD_SIZE ? to_send_sz :
            // TCPConfig::MAX_PAYLOAD_SIZE) :
            //                                   (_window_size < TCPConfig::MAX_PAYLOAD_SIZE ? _window_size :
            //                                   TCPConfig::MAX_PAYLOAD_SIZE);
            ...
            if (_stream.input_ended() && data_size == _stream.buffer_size() && data_size < _window_size)
                fin = true;
            else
                fin = false;
            ...
            // must minus the length by _window_size, because it is possible that fill_window is invoked for many times
            // and no have ack
...
void TCPSender::ack_received(const WrappingInt32 ackno, const uint16_t window_size) {
    ...
    if (absolute_ackno == 1 && state == SYN_SENT)
        state = SYN_ACKED;
    ...
    //       window_size is updated when a ack arrive, in the moment, the SYN already pop, so SYN no occupy the
    //       window_size and the window_size is updated first when a ack for SYN arrive
...
window:
    if (window_size == 0)
        _window_size = 1, sendOneByte = true;
    else
        _window_size = window_size, sendOneByte = false;
